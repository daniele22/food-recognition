...
model = dict(
    ...
    num_stages=3,  # number of stages for HTC
    ...
    backbone=dict(
        ...
        style='pytorch'),
    neck=dict(
        type='FPN',  # the neck of the detector -> feature pyramid network
    ...
    rpn_head=dict(
        ...
        loss_cls=dict(
            type='CrossEntropyLoss',  # Type of loss for classification branch
            use_sigmoid=True, 
            loss_weight=1.0),  
        loss_bbox=dict(type='SmoothL1Loss', beta=1.0 / 9.0, loss_weight=1.0)),  # Config of loss function for the regression branch.
    bbox_roi_extractor=dict(  # RoI feature extractor for bbox regression.
        type='SingleRoIExtractor',
        roi_layer=dict(type='RoIAlign', out_size=7, sample_num=2),
        out_channels=256,
        featmap_strides=[4, 8, 16, 32]),
    bbox_head=[  # Config of box head in the RoIHead
        dict(
