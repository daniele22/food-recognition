...
class Runner(object):
    ...
    def save_checkpoint(self,
                        ...
                        meta=None):
        if meta is None:
            meta = dict(epoch=self.epoch + 1, iter=self.iter, inner_iter=self.inner_iter)
        else:
            meta.update(epoch=self.epoch + 1, iter=self.iter, inner_iter=self.inner_iter)
    ...
    def train(self, data_loader, **kwargs):
        ...
        for _ in range(self._inner_iter):
            next(data_loader)  # skip batches already trained
